...
** How to view mountable directory (nfs protocol) from a host/ip?
#+BEGIN_SRC sh
showmount -e IP
#+END_SRC
...
** How to view mountable directory (CIFS protocol) from a host/ip?
#+BEGIN_SRC sh
smbclient -L //192.168.x.x -U username
#+END_SRC
...
If `NAS server` is unraid, data need to move from `pool` to `array`. It's a must to use option `_netdev,noserverino` due to change in inode!
